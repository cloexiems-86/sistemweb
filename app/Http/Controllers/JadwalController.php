    ...
    public function store(Request $request)
        {
            ...
            $request->validate([
                ...
                'pendamping_id' => 'required|exists:pendampings,id',
...
public function update(Request $request, $id): RedirectResponse
    {
        ...
        $request->validate([
            'jenis_jadwal'  => 'required|in:Bimbingan,Rapak',
            'tanggal'       => 'required|date',
            'topik'         => 'nullable|string|max:255', // Jadi nullable karena Rapak pakai text input (agenda_rapak)
            'sesi'          => 'required|string|max:100', 
            'fasilitator'   => 'required|string|max:255',
            'status'        => 'required|in:Upcoming,Completed',
            'lokasi'        => 'required|string',
            'pendamping_id' => 'nullable|exists:pendamping,id', // Sesuai tabel kamu (pendamping)
        ]);

        $jadwal = Jadwal::findOrFail($id);

        // 2. LOGIKA VALIDASI H-10 KHUSUS RAPAK
        // Hanya dijalankan jika jenisnya Rapak dan ada Catin yang dipilih
